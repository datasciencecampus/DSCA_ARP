...
We will follow the data science workflow: reflect, collect data, prepare data, analyse, report. In tis case study, we will stay within a system, RStudio, for the entire flow.
...
Alternatively, after creating the function `some_function()` we can obtain `combined_data` by

```{r chunk6, eval=F}
combined_data <- excel_sheets(fn) %>% 
map_df(some_function)
```
