...
```{r}
double_up <- function(x){ data_frame(2*x) }
```
...
```{r}
map_dfc(data_frame(1:10, 11:20), double_up)
```

**EXERCISE.** Write a function that when applied to a vector it returns the square root of the sum of squares of the vector entries. Call it `ssq`. Then apply the function to the data frame `data_frame(a = 1:10, b = 11:20, c = 21:30)`. Think about the output of `ssq()`. Solution is below but try to find a solution on your own before looking at the solution.
